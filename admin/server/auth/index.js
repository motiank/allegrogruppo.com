    ...
    users.update(
      {
        user_id: user_id,
        password: password,
        authKey: this.getUserKey(email, password),
      },
      function (err_, tpl_) {
        fn(err_, tpl_);
      },
...
export const cookieSessionStorage = function (req, res, next) {
  ...
  function setUserToCookie() {
    var user = req.user;
    if (user) {
      var ck_str = JSON.stringify({
        user_id: user.user_id,
        email: user.email,
        role: user.role,
        restaurants: Array.isArray(user.restaurants) ? user.restaurants : null,
      });
      var enc_ = encrypt(ck_str);
      res.cookie("user", JSON.stringify(enc_), {
        maxAge: 24 * 3600 * 1000,
        httpOnly: true,
        path: "/",
      });
...
export const router = function (passport) {
                ...
                userApi.setPassword(
                  ...
                  function (err_, tpl_) {
                    return done(null, false, {
                      message: "old format is converted - pls retry",
                    });
                  },
                );
              } else {
                return done(null, false, {
                  message: "Unknown user name or wrong password ",
                });
              }
            } else {
  ...
  function authResponse(req, res) {
    ...
    if (req.flash) obj.flash_error = req.flash("error");
    ...
    if (req.err) obj.meta.err = req.err;
    obj.content.status = "logged-out";
    res.jsonp(obj);
  }
  ...
  router.get(
    ...
    function (req, res, next) {
      ...
      const finish = () => {
        req.user = undefined;
        res.clearCookie("user", { path: "/", httpOnly: true });
        next();
      };
      try {
  ...
  router.post("/changePassword", function (req, res, next) {
    ...
    if (req.body.email) {
      userApi.findByEmail(req.body.email, function (db_res) {
        ...
        if (
          !user ||
          user.authKey != userApi.getUserKey(user.email, req.body.oldPassowrd)
        ) {
          res.json({
            errCode: 104,
            message: "Wrong user or wrong old password",
          });
          return;
        }
